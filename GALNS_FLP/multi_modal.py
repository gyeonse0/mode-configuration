...
while current_num < iteration_num:
    ...
    elif current_num > 5000 and (best_flag == 1 or current_num %50 == 0):
        best_flag = 0
        kopt_state = Rep.k_opt_algorithm(current_states[-1], rnd_state)
        current_states.append(kopt_state)
        objective_value = MultiModalState(kopt_state).cost_objective()
        objectives.append(objective_value)
        time_based_objectives.append(time.time() - start_time)
